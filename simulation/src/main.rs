...
use coffee::graphics::{
    Batch, Color, Frame, Image, Point, Rectangle, Sprite, Vector, Window, WindowSettings,
};
...
impl Game for Particles {
    ...
    fn draw(&mut self, frame: &mut Frame, timer: &Timer) {
        ...
        let sprites = self.particles.par_iter().map(|particle| {
            let velocity = particle.velocity + particle.acceleration * delta_factor;
...
impl Particle {
    fn random<R: Rng>(max_x: f32, max_y: f32, rng: &mut R) -> Particle {
        Particle {
            position: Point::new(rng.gen_range(0.0..max_x), rng.gen_range(0.0..max_y)),
